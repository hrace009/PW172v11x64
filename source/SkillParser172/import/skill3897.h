...
/* 
38970  "Великий взрыв гнева"38971  "^ff5c00Великий взрыв гнева^ffcb4aДальность: ^ffffffближний бой^ffcb4aМаг. энергия: ^96f5ff%d^ffffff^ffcb4aПрименение: ^ffffff1 ^ffcb4aсек.^ffcb4aПерезарядка: ^ffffff3 ^ffcb4aсек.^ffcb4aПолучение ци: ^ffffff0,2^ffcb4aОграничение по оружию: ^ffffffоружие ближнего боя^ffcb4aХарактеристика: ^ff5c00Мощь^ffcb4aСтатус: ^ffffffАсураНаносит противнику базовый физический урон плюс ^96f5ff2825^ffffff ед. физ. урона и вызывает кровотечение,которое через ^96f5ff3^ffffff сек. наносит ему ^96f5ff350%%^ffffff урона оружия плюс ^96f5ff%d^ffffff ед. физического урона.^00ff00Эффект изумрудной руны:@1^00ff001-4 ур.: добавляется ^96f5ff60%%^00ff00 урона от кровотечения^00ff005-7 ур.: добавляется ^96f5ff90%%^00ff00 урона от кровотечения^00ff008-9 ур.: добавляется ^96f5ff120%%^00ff00 урона от кровотечения^00ff00 10 ур.: добавляется ^96f5ff150%%^00ff00 урона от кровотечения@1@2^00ff00Добавляется ^96f5ff%d%%^00ff00 урона от кровотечения.@2"*/ 
...
{ 
          ...
          public:
            ...
            void Calculate (Skill * skill) const
            {
                ...
                skill->SetPlus (skill->GetPlayer ()->GetCurweapon () != 1 ? 4697 : 1839);
 				skill->SetRatio (skill->GetPlayer ()->GetCurweapon () != 1 ? 3.5 : 1.0);
    ...
    bool StateAttack (Skill * skill) const
    {
		...
		static const float LEVEL_5_7 = 0.9;
		static const float LEVEL_8_9 = 1.2;
		...
		float d = (1.0f + skill->GetT0 () - skill->GetPlayer ()->GetDefenddegree () > 0 ? skill->GetT0 () - skill->GetPlayer ()->GetDefenddegree () : 0);
		
		skill->GetVictim ()->SetProbability (1.0 * 100);
        skill->GetVictim ()->SetTime (3000);
        skill->GetVictim ()->SetAmount ((( d * skill->GetT1 ()) + (d *  aarray[skill->GetLevel () - 1])));
