...
#define get_glyph(level, l1_4, l5_7, l8_9, lmax) level >= 1 && level <= 4 ? l1_4 : level >= 5 && level <= 7 ? l5_7 : level >= 8 && level <= 9 ? l8_9 : lmax; 
        ...
        class State3:public SkillStub::State
        {
          public:
            ...
            void Calculate (Skill * skill) const
            {
				float glyph = get_glyph((skill->GetLevel () - 1), 1.20, 1.80, 2.40, 3.00);
				skill->SetSection (1);
				skill->GetPlayer ()->SetDecmp (40); // забирает ману
                skill->SetPlus (11755); // статический урон
                skill->SetRatio (glyph); // урон от оружия
                skill->SetDamage (0.5 * skill->GetAttack ()); // вставляется урон
